...
- [ ] [P2] Profile page for every user type with integrated KYC — ensure each role (User/Customer, Renter, Driver, Admin) has its own profile page reachable from the Navbar; merge the standalone KYC page into the profile page so document upload, verification status, and re-submission live in a KYC section/tab of the profile instead of a separate route; role-aware content: Customer shows license + government ID KYC, Renter shows business permit/registration/tax/bank KYC, Driver shows professional license + background check KYC, Admin shows account settings only (no KYC); redirect the old /kyc route to the profile page KYC section; keep the existing PATCH /users/me endpoints and KYC upload APIs working unchanged — integrate Google Maps JavaScript API on the booking flow pick-up location field; replace free-text input with an interactive map picker (autocomplete search + draggable marker) that stores a precise lat/lng + formatted address; display the pinned location on the booking confirmation ticket
